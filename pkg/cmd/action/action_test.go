...
func TestActionCommand(t *testing.T) {
	...
	t.Run("List-run command flags", func(t *testing.T) {
		...
		listRunCmd, _, err := cmd.Find([]string{"list-run"})
		require.NoError(t, err)

		assert.NotNil(t, listRunCmd.Flag("project"), "Flag --project not found")
		assert.NotNil(t, listRunCmd.Flag("output"), "Flag --output not found")
	})
}
